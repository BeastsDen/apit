...
<xsl:variable
name="partyB"
select="/fpml:NettingInstruction/fpml:swTermination[1]/fpml:swLegalEntity |
/fpml:NettingInstruction/fpml:swNewPosition[1]/fpml:FpML/fpml:party[@id = 'partyB']"/>
...
<xsl:for-each select="fpml:swApiBatchId">
<ApiBatchId><xsl:value-of select="."/></ApiBatchId>
</xsl:for-each>
...
<xsl:for-each select="fpml:swSubCategory">
...
<xsl:for-each select="$partyB[1]/fpml:partyId">
...
<SentBy><xsl:value-of select="fpml:swSentBy"/></SentBy>
</xsl:for-each>
</xsl:template>
<xsl:template match="/">
<header>
