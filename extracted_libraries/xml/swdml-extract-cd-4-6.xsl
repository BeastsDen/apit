...
<xsl:variable name="creditDefaultSwap" select="$trade/fpml:creditDefaultSwap"/>
...
<Tenor>
<xsl:choose>
<xsl:when test="$productType='CDS Index' and                                     not($effectiveDate/fpml:unadjustedDate)">5Y</xsl:when>
<xsl:when test="$productType='CDS Index' and                                      $effectiveDate/fpml:unadjustedDate">
...
<xsl:if test="($productType='CDS Matrix' or $productType='CDS Master') and $periodicPayment/fpml:firstPeriodStartDate">
<xsl:call-template name="formatDate">
...
<FirstPaymentDate>
<xsl:if test="($productType='CDS' or $productType='CDS Matrix' or $productType='CDS Master') and $periodicPayment/fpml:firstPaymentDate">
<xsl:call-template name="formatDate">
...
<xsl:if test="($productType='CDS' or $productType='CDS Matrix' or $productType='CDS Master') and $periodicPayment/fpml:lastRegularPaymentDate">
<xsl:call-template name="formatDate">
...
<FixedRate_2/>
<initialPoints>
<xsl:if test="($productType='CDS Matrix' and $swTradeHeader/fpml:swClientClearing = 'true') or $productType='CDS Master'">
...
<xsl:if test="$swLongFormTrade and $productType='CDS Index'">
...
<xsl:if test="$referenceObligation and                          ($productType='CDS' or                                        $productType='CDS Matrix' or                               $productType='CDS Master')">
<xsl:choose>
...
<RefOblCoupon>
<xsl:if test="($productType='CDS' or $productType='CDS Matrix' or $productType='CDS Master') and $referenceObligation">
...
<Convention>
<xsl:value-of select="$trade//fpml:creditDefaultSwap/fpml:generalTerms/fpml:dateAdjustments/fpml:businessDayConvention"/>
</Convention>
<Holidays>
<xsl:apply-templates select="$trade//fpml:creditDefaultSwap/fpml:generalTerms/fpml:dateAdjustments/fpml:businessCenters"/>
</Holidays>
</CreditDateAdjustments>
<OptionalEarlyTermination>
<xsl:variable name="additionalTerm" select="string($trade//fpml:creditDefaultSwap/fpml:generalTerms/fpml:additionalTerm)"/>
<xsl:value-of select="boolean($additionalTerm='OptionalEarlyTermination')"/>
</OptionalEarlyTermination>
<ReferencePrice>
...
<IndexTradedRate>
<xsl:if test="($productType='CDS Matrix' and $swTradeHeader/fpml:swClientClearing = 'true') or $productType='CDS Index' or $productType='CDS Master'">
...
<UpfrontFeeDate>
<xsl:for-each select="$feeLeg/fpml:initialPayment/fpml:adjustablePaymentDate[$productType='CDS Master' or $productType='CDS Matrix']">
<xsl:call-template name="formatDate"/>
</xsl:for-each>
</UpfrontFeeDate>
<UpfrontFeePayer>
<xsl:if test="$feeLeg/fpml:initialPayment">
<xsl:variable name="payer" select="string($feeLeg/fpml:initialPayment/fpml:payerPartyReference/@href)"/>
<xsl:choose>
...
<PublicationDate>
<xsl:value-of select="$documentation/fpml:contractualTermsSupplement/fpml:publicationDate"/>
</PublicationDate>
<MasterAgreementDate>
...
<xsl:value-of select="$generalTerms/fpml:modifiedEquityDelivery"/>
</ModifiedEquityDelivery>
...
<AdditionalTerms>
<xsl:variable name="med" select="$generalTerms/fpml:additionalTerm[                                       @additionalTermScheme = 'http://derivserv.dtcc.com/coding-scheme/additional-term']"/>
<xsl:choose>
<xsl:when test="count($med) = 0"/>
<xsl:when test="count($med) = 1">
<xsl:value-of select="$generalTerms/fpml:modifiedEquityDelivery"/>
</xsl:when>
<xsl:otherwise>
<xsl:for-each select="$generalTerms/fpml:modifiedEquityDelivery">
