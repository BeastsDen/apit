...
public class RequestRevisionCmd extends DealerCmd {

    public RequestRevisionCmd() {
        super("RequestRevision", "oldDealVersionHandle messageText");
    }

    public static String run(Session session, String oldDealVersionHandle, String messageText) throws ErrorCode {
        System.out.println("RequestRevision - " + oldDealVersionHandle);
        String[] newDealVersionHandle = {new String()};
        DealerAPIWrapper.requestRevision(session.getLoginHandle(), oldDealVersionHandle, messageText, newDealVersionHandle);
