...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:lcl="http://www.markitserv.com/detail/SWDMLTrade.xsl" version="1.0" exclude-result-prefixes="lcl">
  <!--
This file was generated from SWDMLTrade.xsd using the
stylesheet tools/xsl/xsd2xsl.xsl. It should be regenerated whenever
the XSD changes.

Usage:

In the extract stylesheet:

    <xsl:import href="SWDMLTrade.xsl"/>
    <xsl:template match="/">
        <xsl:call-template name="SWDMLTrade">
            <xsl:with-param name="..."/>
        </xsl:call-template>
    </xsl:template>

   Defintions for trade XML element content...
-->
  <xsl:template name="lcl:SWDMLTrade">
    ...
    <xsl:param name="PartyTripartyAgentId" select="/.."/>
    ...
    <SWDMLTrade>
      ...
      <xsl:copy-of select="$PartyTripartyAgentId"/>
  ...
  <xsl:template name="lcl:SWDMLTrade.Allocation">
    ...
    <Allocation>
      ...
      <xsl:if test="$AllocOriginator">
        <AllocOriginator>
          <xsl:value-of select="$AllocOriginator"/>
        </AllocOriginator>
      </xsl:if>
      <xsl:if test="$AllocPlaceofSettleID">
        <AllocPlaceofSettleID>
          <xsl:value-of select="$AllocPlaceofSettleID"/>
        </AllocPlaceofSettleID>
      </xsl:if>
      <xsl:if test="$AllocSettleAgentID">
        <AllocSettleAgentID>
          <xsl:value-of select="$AllocSettleAgentID"/>
        </AllocSettleAgentID>
      </xsl:if>
      <xsl:if test="$AllocSettleAgentSafekeepAC">
        <AllocSettleAgentSafekeepAC>
          <xsl:value-of select="$AllocSettleAgentSafekeepAC"/>
        </AllocSettleAgentSafekeepAC>
      </xsl:if>
      <xsl:if test="$AllocIntermediaryID">
        <AllocIntermediaryID>
          <xsl:value-of select="$AllocIntermediaryID"/>
        </AllocIntermediaryID>
      </xsl:if>
      <xsl:if test="$AllocIntermediarySafekeepAC">
        <AllocIntermediarySafekeepAC>
          <xsl:value-of select="$AllocIntermediarySafekeepAC"/>
        </AllocIntermediarySafekeepAC>
      </xsl:if>
      <xsl:if test="$AllocCustodianID">
        <AllocCustodianID>
          <xsl:value-of select="$AllocCustodianID"/>
        </AllocCustodianID>
      </xsl:if>
      <xsl:if test="$AllocCustodianSafekeepAC">
        <AllocCustodianSafekeepAC>
          <xsl:value-of select="$AllocCustodianSafekeepAC"/>
        </AllocCustodianSafekeepAC>
      </xsl:if>
      <xsl:if test="$AllocBuyerSellerID">
        <AllocBuyerSellerID>
          <xsl:value-of select="$AllocBuyerSellerID"/>
        </AllocBuyerSellerID>
      </xsl:if>
      <xsl:if test="$AllocBuyerSellerSafekeepAC">
        <AllocBuyerSellerSafekeepAC>
          <xsl:value-of select="$AllocBuyerSellerSafekeepAC"/>
        </AllocBuyerSellerSafekeepAC>
      </xsl:if>
