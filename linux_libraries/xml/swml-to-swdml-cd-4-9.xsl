...
<xsl:variable name="xsd-reference">
<xsl:choose>
<xsl:when test="$productType='Offline Trade Credit' or $productType='Offline Trade Credit Summary'">swdml-nc-cd-main-4-9.xsd</xsl:when>
<xsl:otherwise>swdml-cd-main-4-9.xsd</xsl:otherwise>
</xsl:choose>
</xsl:variable>
<xsl:variable name="swdml-version">4-9</xsl:variable>
<SWDML version="{$swdml-version}" xmlns="http://www.fpml.org/2010/FpML-4-9" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.fpml.org/2010/FpML-4-9 {$xsd-reference}">
<swLongFormTrade>
...
<xsl:copy-of select="fpml:swProductSubType"/>
...
<xsl:if test="fpml:swCreditNovationClearing">
<xsl:copy-of select="fpml:swCreditNovationClearing"/>
</xsl:if>
</xsl:if>
<xsl:if test="$productType='Offline Trade Credit'">
<xsl:if test="fpml:swCCPforUnderlyingSwap">
<xsl:copy-of select="fpml:swCCPforUnderlyingSwap"/>
</xsl:if>
</xsl:if>
</xsl:when>
</xsl:choose>
...
<xsl:apply-templates select="fpml:swCreditDefaultSwapDetails/fpml:swCreditDefaultSwapAmendment"/>
...
<xsl:template match="fpml:swCreditDefaultSwapAmendment">
<swCreditDefaultSwapDetails>
<swCreditDefaultSwapAmendment>
<xsl:copy-of select="fpml:swTradeDate"/>
<xsl:copy-of select="fpml:swEffectiveDate"/>
<xsl:copy-of select="fpml:swFeeDetails"/>
</swCreditDefaultSwapAmendment>
</swCreditDefaultSwapDetails>
</xsl:template>
